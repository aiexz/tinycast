...
Selecting a launcher result records **one row for the submitted query** — `submittedQuery`, named so
because a table written when the field held one row per *prefix* cannot decode here, which is the
reset — and recall aggregates every stored query the typed one is a prefix of — so choosing WhatsApp
for `wha` still surfaces it under `w` and `wh`, at a sixteenth of the rows. The 1,000-record cap
therefore holds ~1,000 distinct habits rather than ~60.
...
above an otherwise alphabetical list. Each ordinary palette activation records an overall frecency
entry under `""`; category listings and contextual or fallback rows remain unlearned. When the query
is empty, up to five frequently launched non-favorite entries appear in the Recommended section when
`AppSettings.showRecommendedApps` is enabled. Searches remain upstream-ranked, and the overall
empty-query frecency is used only to order those recommendations. Direct hotkeys and ⌘-digit favorite
